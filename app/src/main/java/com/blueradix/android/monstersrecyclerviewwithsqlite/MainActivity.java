...
//        monstersRecyclerView.setLayoutManager(gridLayoutManager);
    ...
    private void addMonster(Intent data) {
        ...
        if(data.hasExtra(Monster.MONSTER_KEY)) {
            Monster monster = (Monster) data.getSerializableExtra(Monster.MONSTER_KEY);
            ...
            if (result > 0) {
                ...
                message = getString(R.string.activity_main_message_monster_created, result);
            } else {
                message = getString(R.string.activity_main_message_monster_not_created);
            }
